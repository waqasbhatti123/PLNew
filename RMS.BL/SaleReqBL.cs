...
namespace RMS.BL
{
    public class SaleReqBL
    {
        public SaleReqBL()
        { }

        public object wmGetCustomer(string custinfo, RMSDataContext Data)
        {
            try
            {
                if (Data == null) { Data = RMSDB.GetOject(); }

                var heads = (from a in Data.Glmf_Codes
                             where a.gl_cd.StartsWith(custinfo) || a.gl_dsc.Contains(custinfo)
                             select new
                             {
                                 ID = a.gl_cd,
                                 info = a.gl_cd + " / " + a.gl_dsc
                             }).ToList();
        ...
        //public List<spPurchRegRptResult> GetSaleReqMaster(int vrid, char status, RMSDataContext Data)
        //{
        //    if (Data == null) { Data = RMSDB.GetOject(); }
        //    try
        //    {
        //        return Data.spPurchRegRpt(vrid, status).ToList();
        //    }
        //    catch { }
        //    return null;
        //}
        ...
        //    try
        //    {
        //        return Data.spPurchRegDetRpt(vrid, status).ToList();
        //    }
        //    catch { }
        //    return null;
        //}

        public List<spSaleRegRptResult> GetSaleReqDetail(int vrid, char status, RMSDataContext Data)
        {
            if (Data == null) { Data = RMSDB.GetOject(); }
            try
            {
                return Data.spSaleRegRpt(vrid, status.ToString()).ToList();
            }
            catch { }
            return null;
        }
        ...
        public string GetDocNo(DateTime dtTime, RMSDataContext Data)
        {
            ...
            try
            {
                ...
                var records = from n in Data.tblSdSaleReqs
                              //where n.LocId == 5
                              select n;
        ...
        public Object GetSaleReqs(string docNo, string docRefNo, string status, string cust, DateTime fromdt, DateTime todt, RMSDataContext Data)
        {
            if (Data == null) { Data = RMSDB.GetOject(); }

            var purchList = Data.Sp_Filter_Sale_Req(docNo, docRefNo, status,  cust, fromdt, todt).ToList();
        ...
        public tblSdSaleReq GetByID(int vrid, RMSDataContext Data)
        {
            if (Data == null) { Data = RMSDB.GetOject(); }
            return Data.tblSdSaleReqs.Where(p => p.vr_id == vrid).Single();
        }
        public string GetCustomerByID(string custid, RMSDataContext Data)
        {
            if (Data == null) { Data = RMSDB.GetOject(); }
            Glmf_Code glcd = Data.Glmf_Codes.Where(gl => gl.gl_cd == custid).Single();
            return glcd.gl_cd + " / " + glcd.gl_dsc;
        }
        ...
        public string SaveSaleReqFull(tblSdSaleReq poReq, RMSDataContext Data)
        {
            if (Data == null) { Data = RMSDB.GetOject(); }
            ...
            try
            {
                Data.tblSdSaleReqs.InsertOnSubmit(poReq);
                Data.SubmitChanges();
                /*COMMIT*/
                trans.Commit();
        ...
        public string UpdSaleReqFull(tblSdSaleReq poReq, EntitySet<tblSdSaleReqDet> poReqDets, RMSDataContext Data)
        {
            if (Data == null) { Data = RMSDB.GetOject(); }
            /*OPEN AND BEGIN TRANS*/
            System.Data.Common.DbTransaction trans = null;
            ...
            try
            {
                var dets = from p in Data.tblSdSaleReqDets
                           where p.vr_id == poReq.vr_id
                           select p;

                Data.tblSdSaleReqDets.DeleteAllOnSubmit(dets);

               // poReq.tblSdSaleReqDets = poReqDets;
