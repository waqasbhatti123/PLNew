...
{
    public partial class ExpenditureStatement : BasePage
    {
        ...
        {
            return new BudgetBL().GetExpenditure((RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"], BudgetTypeID, Financialyear, brID);
        }
        ...
        public static string SaveBudget(List<RMS.BL.Budget> budget)
        {
            try
            {
                ...
                string Chk = budgetBL.SubmitExpenditure(Financialyear, BudgetTypeID, brId, budget, (RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"]);
                if (Chk == "OK")
                {
