...
{
    public partial class ExcessSsurrendertwo : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if(BudgetTypeID == 1)
                {
                    Session["PageTitle"] = "Excess & Surrenders Two";
                }
        ...
        {
            return new BudgetBL().GetExcessSurrender2((RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"], BudgetTypeID, Financialyear, brID);
        }
        ...
        [WebMethod]
        [System.Web.Script.Services.ScriptMethod]
        public static string SaveBudget(List<RMS.BL.ExcessSurrender> excess)
        {
            try
            {
                BudgetBL budgetBL = new BudgetBL();
                int brId = Convert.ToInt32(excess[0].brId);
                string Chk = budgetBL.SubmitExcess2(Financialyear, brId, 2 ,excess, (RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"]);
                if (Chk == "OK")
                {
