...
namespace RMS.GLSetup
{
    ...
    public partial class frmLedgerCard : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                txtFrom.Text = cuurentYear.fromFinalYearDate((RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);  //new DateTime(Common.MyDate((RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]).Year, Common.MyDate((RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]).Month, 1).ToString(Request.Cookies["uzr"]["DateFullYearFormat"]);
                txtTo.Text = cuurentYear.toFinalYearDate((RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);//Common.MyDate((RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]).ToString(Request.Cookies["uzr"]["DateFullYearFormat"]);
                ...
                if (PID == 332)
                {
                    Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "LedgerCard").ToString();
                }
                if (PID == 334)
                {
                    Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "TB").ToString();
                }
                FillSearchBranchDropDown();
                searchBranchDropDown.SelectedValue = BranchID.ToString();

                FillHeadsDetail(BranchID);
        ...
        private void FillHeadsDetail(int br)
        {
            RMSDataContext db = new RMSDataContext();

            PreferenceBL bl = new PreferenceBL();

            var data = (from gc in db.Glmf_Codes
                        join v in db.glmf_ven_cus_dets on gc.gl_cd equals v.gl_cd
                        into gl
            ...
            codeDropDown.DataSource = data.ToList();
        ...
        protected void btnGenerat_Click(object sender, EventArgs e)
        {
            if (PID == 332)
            {
                CreatePDF("LedgerCardBal", "pdf", txtFrom.Text, txtTo.Text, codeDropDown.SelectedValue, "");
            }
            if (PID == 334)
            {
                CreatePDF("TrialBalance", "pdf", txtFrom.Text, txtTo.Text, codeDropDown.SelectedValue, "");
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
            ...
            //List<sp_LedgerResult> sal = cty.GetLedgerBal(BranchID, dtFrm, dt2, fromcode, tocode,fnObject.Gl_Year,Convert.ToChar(ddlStatus.SelectedValue), (RMSDataContext)Session[Session["UserID"] + "rmsDBobj"],'A' );

            List<sp_Leadgerdbo> sal = pucarBl.LedgerReports(BranchID, fnObject.Gl_Year, dtFrm, dt2, 'A', Convert.ToChar(ddlStatus.SelectedValue), fromcode, tocode, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]).ToList();
            decimal dbt = 0;
            decimal cred = 0;
            //string glcd = "";
            decimal tot = 0;
