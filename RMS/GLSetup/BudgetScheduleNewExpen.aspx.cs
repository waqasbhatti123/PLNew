...
{
    public partial class BudgetScheduleNewExpen : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if(BudgetTypeID == 3)
                {
                    Session["PageTitle"] = "Schedule of new Expenditure(SNE)";
                }
        ...
        {
            return new BudgetBL().GetSNE((RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"], BudgetTypeID, Financialyear, brID);
        }
        ...
        public static string SaveBudget(List<RMS.BL.Budget> budget)
        {
            try
            {
                ...
                string Chk = budgetBL.SubmitSNE(Financialyear, BudgetTypeID, brId, budget, (RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"]);
                if (Chk == "OK")
                {
