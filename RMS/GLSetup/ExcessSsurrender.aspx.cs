...
{
    public partial class ExcessSsurrender : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if(BudgetTypeID == 1)
                {
                    Session["PageTitle"] = "Excess & Surrenders";
                }
        ...
        {
            return new BudgetBL().GetExcessSurrender1((RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"], BudgetTypeID, Financialyear, brID);
        }
        ...
        [WebMethod]
        [System.Web.Script.Services.ScriptMethod]
        public static string SaveBudget(List<RMS.BL.ExcessSurrender> excesses)
        {
            try
            {
                BudgetBL budgetBL = new BudgetBL();
                int brId = Convert.ToInt32(excesses[0].brId);
                string Chk = budgetBL.SubmitExcess1(Financialyear, brId, BudgetTypeID ,excesses, (RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"]);
                if (Chk == "OK")
                {
