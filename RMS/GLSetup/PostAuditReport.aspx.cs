...
{
    public partial class PostAuditReport : BasePage
    {
        ...
        protected void CreatePDF(String FileName)
        {
            ...
            int exactyear = Convert.ToInt32(yr[1]);
            // Variables
           // int fin = Convert.ToInt32(Financialyear);
            ...
            Branch brr = data.Branches.Where(x => x.br_id == br_id).FirstOrDefault();

            IList<Sp_ExpenditureStatementHeadWiseResult> sal;
            sal = exp.GetExpenditureStatement(br_id, exactyear,(RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);

            viewer.LocalReport.ReportPath = "glsetup/rdlc/PostAuditReport.rdlc";
