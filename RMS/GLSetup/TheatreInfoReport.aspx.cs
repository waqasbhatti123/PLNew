...
{
    public partial class TheatreInfoReport : System.Web.UI.Page
    {
        RMSDataContext db = new RMSDataContext();
        NewExpenditure exp = new NewExpenditure();

#pragma warning disable CS0114 // 'TheatreInfoReport.ID' hides inherited member 'Page.ID'. To make the current member override that implementation, add the override keyword. Otherwise add the new keyword.
        public int ID
#pragma warning restore CS0114 // 'TheatreInfoReport.ID' hides inherited member 'Page.ID'. To make the current member override that implementation, add the override keyword. Otherwise add the new keyword.
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "thInforReport").ToString();
        ...
        protected void Click_Generate(object sender, EventArgs e)
        {
            int div = Convert.ToInt32(ddlDivisional.SelectedValue);

            Branch br = db.Branches.Where(x => x.br_id == div && x.br_status == true).FirstOrDefault();
            // Variables
            string rptLogoPath = "";
            ...
            IList<sp_TheatreInformationResult> theatre = exp.GetTheInfo(div, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]).ToList();
