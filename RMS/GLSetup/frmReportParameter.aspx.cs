...
namespace RMS.GL.Setup
{
    ...
    public partial class frmReportParameter : System.Web.UI.Page
    {

        #region DataMember
        
        EntitySet<tblglrptHdr> glrptDetEntSet = new EntitySet<tblglrptHdr>();
            ...
            try
            {
                if (glParameter.isExist(compId, Convert.ToInt16(txtReportNo.Text), (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]) == false)
                {
                    ...
                    for (int i = 0; i < grdView.Rows.Count; i++)
                    {
                        ...
                        {
                            tblglrptHdr rptDet = new tblglrptHdr();
                            ...
                            rptDet.ActTo = AcTo;
                            if (CcFr != "")
                                rptDet.CC_Grp = Convert.ToInt16(CcFr);
                            else
                                rptDet.CC_Grp = null;
                    ...
                    //if ()
                    //{
                    if (glParameter.saveHdr(glrpt, glrptDetEntSet, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]))
                    {
        ...
        private void edit()
        {
            try
            {
                ...
                //---------Deleting records
                glParameter.deleteHdrRecord(compId, rptNo, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
                //----------
                if (glParameter.saveHdr(glrpt, glrptDetEntSet, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]))
                {
                    ucMessage.ShowMessage("Saved Successfully...", RMS.BL.Enums.MessageType.Info);
        ...
        private void getByRptNumber()
        {
            ...
            getColumn();
            List<tblglrptHdr> ls = glParameter.getDetailByRptNoHdr(rptNo, compId, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            foreach (var a in ls)
            {
                drow = d_table.NewRow();
                drow["RecNo"] = a.RecNo;
