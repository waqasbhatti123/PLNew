...
namespace RMS.GL.Setup
{
    ...
    public partial class frmFinancialReportRpt_New : System.Web.UI.Page
    {
        ...
        protected void printRpt(int reportN, DateTime toDate)
        {
            ...
            try
            {
                ...
                List<spHdrGlReportResult> ls = glParameter.getHdrGlReportData(compId, brId, reportN, toDate, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
                foreach (var a in ls)
                {
                    if (a.RecType == 'E')
                    {
        ...
        private void GetSeriesRecs(spHdrGlReportResult a, DateTime toDate)
        {
            DataRow dataRow;
            List<spGlReportSeriesResult> ls = glParameter.getGlReportSeriesData(brId, toDate, a.ActFrom, a.ActTo, null, null, 'A', (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            //-----------for skip--
            if (a.SkipB4 != 0)
            {
                int i = 0;
