...
{
    public partial class GrantRelease : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
               Session["PageTitle"] = "Grant Release";
        ...
        //{
        //    Response.Redirect("GrantReleaseReport.aspx");
        //}
        ...
        {
            return new BudgetBL().GetGrantRelease((RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"], BudgetTypeID, Financialyear, brID);
        }
        ...
        [WebMethod]
        [System.Web.Script.Services.ScriptMethod]
        public static string SaveBudget(List<BudgetModel> budget)
        {
            try
            {
                BudgetBL budgetBL = new BudgetBL();
                int brId = Convert.ToInt32(budget[0].br_id);
                string Chk = budgetBL.SubmitRelease(Financialyear, BudgetTypeID, brId, budget, (RMSDataContext)HttpContext.Current.Session[HttpContext.Current.Session["UserID"] + "rmsDBObj"]);
                if (Chk == "OK")
                {
