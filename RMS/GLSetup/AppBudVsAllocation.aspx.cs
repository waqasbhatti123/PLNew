...
{
    public partial class AppBudVsAllocation : System.Web.UI.Page
    {
        RMSDataContext db = new RMSDataContext();
        NewExpenditure exp = new NewExpenditure();

#pragma warning disable CS0114 // 'AppBudVsAllocation.ID' hides inherited member 'Page.ID'. To make the current member override that implementation, add the override keyword. Otherwise add the new keyword.
        public int ID
#pragma warning restore CS0114 // 'AppBudVsAllocation.ID' hides inherited member 'Page.ID'. To make the current member override that implementation, add the override keyword. Otherwise add the new keyword.
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "Appexp").ToString();
        ...
        protected void Report_Click(object sender, EventArgs e)
        {
            ...
            IList<sp_AppBudVsExpenditureResult> expp =exp.GetAppExp(yearr,div, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]).ToList();

            viewer.LocalReport.ReportPath = "glsetup/rdlc/rptAppExp.rdlc";
