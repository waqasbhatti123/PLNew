...
namespace RMS
{
    public partial class RawHideStockReport : BasePage
    {
        #region DataMembers
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (PID == 451)
                {
                    Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "RawHideStockReport").ToString();
        ...
        protected void rdbStockAll_Changed(object sender, EventArgs e)
        {

        }

        protected void rdbStockYes_Changed(object sender, EventArgs e)
        {

        }

        protected void rdbStockNo_Changed(object sender, EventArgs e)
        {
        ...
        {
            if (rdbStockAll.Checked == true)
            {
                getStockAll();
            }
            else if(rdbStockNo.Checked==true || rdbStockYes.Checked==true)
            {
                getStockByStatus();
            }
        }
        ...
        public void getStockAll()
        {
            ...
            reportViewer.LocalReport.ReportPath = "InvReports/rdlc/RawHideStockByStatusRpt.rdlc";
            ...
            List<spRawHideStockByStatusResult> hideStock = rptBl.GetRawHideStockLstByStatus(fromDt, toDt,"",  (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);


            ReportDataSource dataSource = new ReportDataSource("spRawHideStockByStatusResult", hideStock);

            ReportParameter[] rpt = new ReportParameter[3];
            rpt[0] = new ReportParameter("ReportName", "Raw Hide Stock Report");
            ...
            filename = string.Format("{0}.{1}", "RawHideStockReport", "pdf");
        ...
        public void getStockByStatus()
        {
            DateTime fromDt = Convert.ToDateTime(txtfromDt.Text);
            DateTime toDt = Convert.ToDateTime(txttoDt.Text);
            string sts = null;
            if (rdbStockYes.Checked == true)
            {
                sts = "Yes";
            }
            else if (rdbStockNo.Checked == true)
            {
                sts = "No";
            }
            ...
            List<spRawHideStockByStatusResult> hideStock = rptBl.GetRawHideStockLstByStatus(fromDt, toDt,sts, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
