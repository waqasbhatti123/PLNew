...
namespace RMS.sales
{
    public partial class RecSettlement : BasePage
    {
        #region DataMembers

        RecSettlementBL RecBL = new RecSettlementBL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "RecSettlement").ToString();
                FillDdlVendor();
            }
        }
        ...
        protected void btnSave_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (!isInvChecked)
            {
                ucMessage.ShowMessage("Please check atleast one receipt to continue", BL.Enums.MessageType.Error);
                return;
            }
            ...
            if (grdInvoices.Rows.Count > 0 && isInvChecked && grdPayments.Rows.Count > 0 && isPayChecked)
            {
                ...
                string msg = RecBL.SaveSettlements(entBill, entSet, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
                if (msg == "Done")
                {
                    BindGridInvoices();
                    BindGridPayments();
        ...
        {
            grdInvoices.DataSource = RecBL.GetAllBillByVendorID(ddlVendor.SelectedValue, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            grdInvoices.DataBind();
        }
        private void BindGridPayments()
        {
            grdPayments.DataSource = RecBL.GetAllPaymentsByVendorID(ddlVendor.SelectedValue, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
