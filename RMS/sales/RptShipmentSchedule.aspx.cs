...
namespace RMS.sales
{
    public partial class RptShipmentSchedule : System.Web.UI.Page
    {
        #region DataMember
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "ShipSchedule").ToString();
        ...
        protected void btnGenerat_Click(object sender, EventArgs e)
        {
            try
            {
                GenerateReport("ShipmentSchedule");
            }
            catch (Exception ex)
            {
        ...
        protected void GenerateReport(string reportName)
        {
            ...
            List<spShipmentScheduleResult> shipmentschedule;
            shipmentschedule = slBL.GetShipmentSchedule(fromDate, toDate, Convert.ToInt32(ddlSorting.SelectedValue), (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            if (ddlSorting.SelectedValue == "0")//Sort by item
                viewer.LocalReport.ReportPath = "sales/rdlc/ShipScheduleByItem.rdlc";
            else
                viewer.LocalReport.ReportPath = "sales/rdlc/ShipScheduleByParty.rdlc";
            ReportDataSource datasource = new ReportDataSource("spShipmentScheduleResult", shipmentschedule);
