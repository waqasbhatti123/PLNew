...
namespace RMS
{
    public partial class RMSMasterHome : System.Web.UI.MasterPage
    {
        #region DataMembers
        ...
        CuurentYearBL cur = new CuurentYearBL();
        ...
        public void GetFin()
        {
            var fin = cur.GetFin((RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            DateTime start = Convert.ToDateTime(fin.Start_Date);
            DateTime End = Convert.ToDateTime(fin.End_Date);
            int star = start.Year;
            int en = End.Year;
            this.lblYear.Text = star + "-" + en;
        }
