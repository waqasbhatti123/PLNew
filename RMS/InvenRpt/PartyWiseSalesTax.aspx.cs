...
{
    public partial class PartyWiseSalesTax : BasePage
    {
        #region DataMembers
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (PID == 538 || PID == 887)
                {
                        Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "PartywiseSTRpt").ToString();
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                reportViewer.LocalReport.ReportPath = "InvenRpt/rdlc/PartyWiseSalesTax.rdlc";
                reportViewer.ProcessingMode = Microsoft.Reporting.WebForms.ProcessingMode.Local;
                reportViewer.LocalReport.Refresh();
                reportViewer.LocalReport.EnableExternalImages = true;
                reportViewer.LocalReport.Refresh();

                List<spPartywiseSalesTaxResult> sTax = rptBl.GetPartyWiseSalesTax(BrId, Convert.ToInt32(ddlLoc.SelectedValue), fromDt, toDt, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);

                ReportDataSource dataSource = new ReportDataSource("spPartywiseSalesTaxResult", sTax);

                ReportParameter[] rpt = new ReportParameter[3];
                rpt[0] = new ReportParameter("ReportName", "Party Wise Sales Tax");
                ...
                filename = string.Format("{0}.{1}", "Party Wise Sales Tax", ext);
