...
{
    public partial class ItemWiseStockMovementReport : BasePage
    {
        #region DataMembers
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (PID == 522)
                {
                    Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "InvenStockMovementReport").ToString();
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                reportViewer.LocalReport.ReportPath = "InvenRpt/rdlc/ItemWiseStockMovement.rdlc";
                ...
                List<sp_InvMonthlyStatusResult> StkMovement = rptBl.getItemWiseStkMovementSummary(BrId, ddlItemGroup.SelectedValue, Convert.ToDecimal(ddlLoc.SelectedValue), fromDt, toDt, txtcodefrom.Text, txtcodeto.Text, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);


                ReportDataSource dataSource = new ReportDataSource("sp_InvMonthlyStatusResult", StkMovement);

                ReportParameter[] rpt = new ReportParameter[3];
                rpt[0] = new ReportParameter("ReportName", "Item-Wise Stock Movement Report");
                ...
                filename = string.Format("{0}.{1}", "StockMovementReport", ext);
