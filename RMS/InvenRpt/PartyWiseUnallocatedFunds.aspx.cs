...
{
    public partial class PartyWiseUnallocatedFunds : BasePage
    {
        #region DataMembers
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (PID == 536 || PID == 885)
                {
                    Session["PageTitle"] = GetGlobalResourceObject("PageTitlesResource", "PWF").ToString();
                    FillDropDownVendor();
                }
            }
        }
        ...
        {
            string ddlVal = ddlFunds.SelectedValue;
            int BrId = 0;
            ...
            getUnAllocatedInvoice(ddlVal, BrId);             
        ...
        {
            ddlFunds.DataSource = new VendorBL().GetVendor((RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            ddlFunds.DataTextField = "gl_dsc";
            ddlFunds.DataValueField = "gl_cd";
            ddlFunds.DataBind();
        }
        public void getUnAllocatedInvoice(string val, int brid)
        {
            ReportViewer reportViewer = new ReportViewer();
            reportViewer.Visible = false;
            reportViewer.LocalReport.ReportPath = "InvenRpt/rdlc/PartyWiseUnallocatedFundsRpt.rdlc";
...
#pragma warning restore CS0219 // The variable 'tp' is assigned but its value is never used
            List<spPartyWiseUnallocatedFundsResult> unAllocatedFunds = rptBl.getUnallocatedFunds(val,brid, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);

            ReportDataSource dataSource = new ReportDataSource("spPartyWiseUnallocatedFundsResult", unAllocatedFunds);
