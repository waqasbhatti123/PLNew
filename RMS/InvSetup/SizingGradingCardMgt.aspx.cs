...
namespace RMS
{
    public partial class SizingGradingCardMgt : BasePage
    {
        #region DataMembers
...
#pragma warning disable CS0414 // The field 'SizingGradingCardMgt.flagClear' is assigned but its value is never used
        bool flagClear = false;
#pragma warning restore CS0414 // The field 'SizingGradingCardMgt.flagClear' is assigned but its value is never used
        #endregion
            ...
            if (igpNo > 0)
            {
                try
                {
                    ...
                    if (grdIGP.Rows.Count > 0)
                    {
                        ...
                        sizedQty = sG.GetSizedQty(igpNo, 11, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
                        for (int i = 0; i < GridView1.Rows.Count; i++)
                        {
