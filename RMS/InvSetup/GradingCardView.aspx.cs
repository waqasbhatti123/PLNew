...
namespace RMS
{
    public partial class GradingCardView : BasePage
    {
        #region DataMembers
                ...
                if (txtLotNo.Text != "")
                {
                    try
                    {
                        ...
                        else
                        {
                            txtQty.Text = "";
                            ddlProduct.SelectedValue = "0";
                            ucMsgPnl.ShowMessage("Card already exists.", RMS.BL.Enums.MessageType.Error);
                        }
                    }
                    catch
                    {
        ...
        public void Edit()
        {
            try
            {
                ...
                if (sumQty + grdQty <= mtQty)
                {
                    ...
                    int yr = Convert.ToInt32(Session["DateYear"]);
                    string vrNo = Session["vrNO"].ToString();
                    bool res = gC.EditRec(sgNo, Convert.ToInt32(vrNo.Substring(0, 4)), 15, itmData, itmDetEnt, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
                    if (res == true)
                    {
