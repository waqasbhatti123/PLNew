...
{
    public partial class InwardGatePassMgt : BasePage
    {
        #region DataMembers
                ...
                if (gpRefExist == false)
                {
                    ...
                    if (sumQty == Convert.ToInt32(txtTotalQty.Text))
                    {
                        for (int i = 0; i < GridView1.Rows.Count; i++)
                        {
                            ...
                            if (gpRef != "" && qty != "" && prty != "0")
                            {
                                tblStkGP stkGp = new tblStkGP();/*=tblStkGP================================================*/
                                ...
                                rowCount++;
                                tblStkGPDet stkGpDet = new tblStkGPDet();/*=tblStkGPDet=============================================*/
                                if (Session["BranchID"] == null)
                                ...
                                if (((System.Web.UI.WebControls.TextBox)GridView1.Rows[i].FindControl("txtWeight")).Text != "")
                                    stkGpDet.Price = Convert.ToDecimal(((System.Web.UI.WebControls.TextBox)GridView1.Rows[i].FindControl("txtWeight")).Text);
                                else
                                    stkGpDet.Price = 0;
                                stkGpDet.KgsWt = 0;
                    ...
                    if (sumQty == Convert.ToInt32(txtTotalQty.Text))
                    {
                        ...
                        for (int i = 0; i < GridView1.Rows.Count; i++)
                        {
                            ...
                            if (gpRef != "" && qty != "" && prty != "0")
                            {
                                ...
                                string no = txtGPNo.Text.Substring(5);
                                string yrno = txtGPNo.Text.Substring(0, 4).ToString() + Convert.ToString(Convert.ToInt32(no) + count);
                                stkGp.vr_no = Convert.ToInt32(yrno);
                                ...
                                if (rowCount > 0)/*=Save GP==============================================*/
                                {
                                    ...
                                    {
                                        ucMessage.ShowMessage(ez.Message, RMS.BL.Enums.MessageType.Error);
                                        return;
                                    }
                                }
                                else
                        ...
                        if (res == true)
                        {
                            ...
                            Response.Redirect("~/invsetup/inwardgatepasshome.aspx?PID=421",false);
                           
                        }
                        else
                        {
                            ucMessage.ShowMessage("Save was unsuccessful as row data is not correctly entered.", RMS.BL.Enums.MessageType.Error);
                        }
                    }
                    else
                    {
