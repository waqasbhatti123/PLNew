...
namespace RMS
{
    public partial class MaterialPurchaseNoteViewNew : BasePage
    {
...
#pragma warning disable CS0169 // The field 'MaterialPurchaseNoteViewNew.d_Row' is never used
        DataRow d_Row;
#pragma warning restore CS0169 // The field 'MaterialPurchaseNoteViewNew.d_Row' is never used
#pragma warning disable CS0169 // The field 'MaterialPurchaseNoteViewNew.d_Col' is never used
        DataColumn d_Col;
#pragma warning restore CS0169 // The field 'MaterialPurchaseNoteViewNew.d_Col' is never used
            ...
            if (!IsPostBack)
            {
                ...
                if (PID == 435)
                {
                    ...
                    else
                    {
                        ...
                        btnSave.Visible = false;
                        btnList.Visible = true;
                        lblVvDet.Attributes.Add("onclick", "window.open('../InvSetupSupport/ViewIgp.aspx?ID=" + Doc_RefFirst + "','IGPDetail','height=400,width=700,toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, directories=no, status=no');return false");
                    }
                }
            }
        ...
        protected void LnkSelectVendor_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    BindGridIGPs(null);
                    uMsg.ShowMessage("Nothing to purchase against this vendor.", RMS.BL.Enums.MessageType.Error);
                }
            }
            catch (Exception ex)
            {
        ...
        protected void grdGrading_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                e.Row.Cells[4].Text = Math.Round(Convert.ToDecimal(e.Row.Cells[2].Text) * 100 / Convert.ToDecimal(QuantityGrd), 3).ToString();
                e.Row.Cells[5].Text = Math.Round(Convert.ToDecimal(e.Row.Cells[4].Text) * AreaFtg / 100, 2).ToString();
        ...
        {
            ddlSelectVendor.DataSource = new VendorBL().GetVendor((RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            ddlSelectVendor.DataTextField = "gl_dsc";
            ddlSelectVendor.DataValueField = "gl_cd";
