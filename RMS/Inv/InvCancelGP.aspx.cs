...
namespace RMS.Inv
{
    public partial class InvCancelGP : BasePage
    {
        ...
        protected void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            string msg = igpBL.checkIGP(VrNO, brid, VtCD, LocID, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            if (msg == "OK")
            {
                string obj = igpBL.cancelIGP(VrNO, brid, VtCD, LocID, (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
                if (obj == "OK")
                {
                    ucMessage.ShowMessage("GP Cancelled Successfully.", RMS.BL.Enums.MessageType.Info);
            ...
            {
                ucMessage.ShowMessage("GP exist in material reciept note, Please cancel MRN " + msg + " to continue", RMS.BL.Enums.MessageType.Error);
                //ClearFieldsOnly();
                //BindGridMain("", "", 0, "0");
            }
        ...
        {
            Response.Redirect("~/inv/InvCancelGP.aspx?PID=564");
        }
        ...
        private void BindGridMain(string igpNo, string party, int cityId, string status)
        {
            ...
            grdIgp.DataSource = igpBL.GetAllChemIGPs(igpNo, party, cityId, "A", Convert.ToInt32(ddlLoc.SelectedValue), (RMSDataContext)Session[Session["UserID"] + "rmsDBObj"]);
            grdIgp.DataBind();
        }
